  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <Suspense fallback={<div>Loading repositories and pull requests...</div>}>
            <ul className="space-y-8">
              {reposWithPRs.map((repo) => (
                <li key={repo.id} className="border-b pb-6">
                  <h3 className="font-semibold text-lg mb-2">{repo.name}</h3>
                  <p className="text-sm text-gray-600 mb-4">Builds: Coming soon</p>
                  ...
                  {repo.pullRequests.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-600 pl-4">No open pull requests</p>
                  )}
                </li>
              ))}
            </ul>
